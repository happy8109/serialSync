...
## SerialManager 对外接口说明

### 主要方法

| 方法 | 说明 | 参数 | 返回值 |
|------|------|------|--------|
| async connect(portOverride?) | 连接串口 | portOverride: string（可选，覆盖端口） | Promise<void> |
...
| progress | 文件分块进度 | {type, seq, total} |
| error | 错误事件 | Error |
| connected | 串口已连接 | 无 |
...
sm.on('progress', ({type, seq, total}) => {
  console.log(`进度: ${type} ${seq+1}/${total}`);
});
sm.on('error', (err) => {
