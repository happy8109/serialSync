...
## 文件同步功能开发任务清单（2025-07-15）

### 1. 基础链路（MVP）优先级
1. 递归扫描本地目录，生成文件状态清单（含 path、mtime、size/hash）
2. 串口发送/接收文件状态清单（JSON），实现双方状态交换
3. 实现本地与对方清单的差异比对算法，输出需同步的文件列表
4. 基础同步任务调度（单任务、单向、递归、最近修改覆盖）
5. 按需发起文件传输（复用现有分块协议），支持递归目录和自动创建子目录
6. 文件传输完成后落地，更新本地状态，支持“最近修改覆盖”冲突策略
7. 日志记录同步操作、冲突、覆盖，WEB UI 显示同步进度
8. WEB UI 增加“发起同步”按钮或命令，便于手动触发

### 2. 队列与任务管理扩展
9. 设计并实现 FileTransferQueue，统一管理所有文件传输任务
10. 支持多任务管理（新建、修改、启用、停止、删除），任务包含路径、方向、策略、优先级、周期等
11. 队列支持任务优先级调度，高优先级任务先执行
12. 支持定时/周期同步（如每10分钟自动同步），任务可配置 schedule 字段
13. 支持任务暂停、恢复、取消、状态查询
14. 支持同步历史、失败重试、详细日志

### 3. 进阶与优化
15. 支持同步排除规则、过滤特定文件/目录
16. 支持同步冲突日志、覆盖提示
17. 支持任务分组、批量管理
18. 支持更复杂的冲突解决策略（如保留副本、人工确认）
19. 优化 UI/CLI 交互体验，完善同步进度、历史、错误提示
