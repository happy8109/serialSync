...
## 7. 跨网中继机制 (Cross-Network Relay Protocol) - v3.1

### 7.1 拓扑模型与链式级联

在物理串口与局域网混合的多网环境下，系统支持任意数量网络（A网、B网、C网...）通过中继节点以链式级联拓扑结构进行组网。
* **串口对 (Serial Pair)**：物理隔离网络之间通过 RS-232/485 串口连接。
* **局域网中继对 (LAN Relay Pair)**：同一网络内两台摆渡主机之间建立 WebSocket 直连连接（默认路径为 `/relay`）。
...
| TYPE | 名称 | 说明 | Body 格式 |
| :--- | :--- | :--- | :--- |
| `0x15` | FILE_NOTIFY | 文件广播通知 | JSON: `{msgId, fileId, fileName, fileSize, sender}` |
| `0x50` | FILE_RELAY_REQ | 中继拉取请求 | JSON: `{reqId, fileId, fileName}` |
| `0x51` | FILE_RELAY_RESP | 中继拉取响应 | JSON: `{reqId, fileId, status, error}` |

### 7.3 消息转发与防环路机制

消息中继同时在串口（`MessageService`）和局域网（`RelayService`）层级执行去重，实现高容错无环路广播：
1. **msgId 溯源去重**：发送端生成全局唯一 `msgId`。每个节点均维护最近 500 条消息的 `seenMsgIds` 缓存。当收到重复的 `msgId` 时直接丢弃，不向下一跳扩散。
2. **反向抑制与发送剪枝**：局域网对端之间在握手时交换各自的 `nodeName`。节点转发消息给局域网对端时，若消息的原发送者 `nodeName` 与对端匹配，则不予发送。

### 7.4 文件中继拉取协调

当用户拉取文件时，中继机制根据文件路径分流处理，对用户完全透明：
* **局域网段**：如果目标文件在局域网内其他节点上，拉取响应直接返回 HTTP 下载 URL（`/api/relay/file/:fileId`），通过千兆局域网执行秒级拉取。
* **串口网段**：如果目标文件在远端隔离网内，拉取请求通过串口（`FILE_RELAY_REQ`）逐跳转发，远端节点作为发送端通过串口启动传输。传输完成落地后，再沿中继路径回传，直至发起拉取的本地 WebUI 节点。
* **中继状态机**：`RelayService` 维护 `pendingRelayPulls` 状态表。记录拉取请求的方向（来自串口还是本地 WebUI）。文件分片落地后，自动根据方向将文件转发（注入串口发送或通知本地 WebUI 下载）。

### 7.5 用户身份标识

消息与文件通知的 `sender` 信息包含：
* `nodeName`：发送者所在中继主机的主机名。
* `network`：发送者所在的网络标识（可在配置文件中自定义）。
* `nickname`：WebUI 用户自定义昵称。
* `ip`：客户端局域网 IP。

在聊天界面中，其他网络的消息发送者渲染为 `[网络名] 昵称` 格式，保障端到端可追溯性。
