...
接下来的重点是完善 Web UI，实现 IM 风格的消息融合与持久化。
...
- [ ] **数据持久化实现**
    - [ ] 修改 `appStore.js` 引入 `zustand/persist` 中间件
    - [ ] 配置 `messages` 和 `transfers` 的持久化存储
    - [ ] 实现旧数据的清理与兼容逻辑

- [ ] **UI 布局与交互重构**
    - [ ] 改造 `ChatView.jsx` 布局为 IM 样式 (左收右发)
    - [ ] 增加附件按钮与拖拽发送文件支持 (`Dropzone` 集成)
    - [ ] 取消独立的 "File Transfer" 标签页

- [ ] **消息气泡组件化**
    - [ ] 封装 `ChatMessage` 通用气泡容器
    - [ ] 开发 `FileBubble` 组件 (展示实时传输进度、速度及控制按钮)
    - [ ] 处理系统消息显示 (串口状态变化等)

- [ ] **逻辑关联与同步**
    - [ ] 实现发送/接收文件时自动在 `messages` 中插入 `file` 类型消息
    - [ ] 确保 `FileBubble` 能够正确订阅后台传输进度
    - [ ] 处理传输任务失败、取消后的 UI 反馈
